/* Cover warp jump. The canvas backing store is one pixel per block and CSS
   ...
   The jump runs once on load and then comes to rest. At rest the field keeps
   living — stars breathe and are retired and replaced — but nothing moves
   across the grid, because below one block of travel per frame a star holds
   still and then jumps a whole block, which is what made the old slow drift
   stutter. Brightness has no such floor, so resting costs nothing to look at.
   Under reduced motion a single static field is drawn instead. */
...
(function () {
  ...
  /* at rest a star is a single block rather than a streak, so it needs more
     weight than the moving field to read as the same starfield */
  var REST_ALPHA = 1, REST_FLOOR = 0.3;
  ...
  var resting = false, restAcc = 0, motionless = false;
  var REST_STEP = 0.07;                                // ~14 redraws a second at rest
  ...
  function star(r) {
    return {
      ...
      tw: Math.random() * Math.PI * 2,        // breath phase, used at rest
      tr: 0.5 + Math.random() * 1.4,          // breath rate
      fd: 1,                                  // 0 while being replaced
      dy: false
    };
  }
  ...
  function frame(now) {
    ...
    if (resting) {
      /* a breath does not need 60fps, and redrawing less often keeps the page
         idle enough to read on top of */
      restAcc += dt;
      if (restAcc >= REST_STEP) {
        renderRest(restAcc);
        restAcc = 0;
      }
    } else {
      render(t, dt);
      if (elapsed >= TOTAL) {               // arrived: hand over to the rest loop
        resting = true;
        restAcc = 0;
        announce();
  ...
  /* At rest nothing travels. Stars breathe, and a few at a time are retired
     and replaced somewhere else, fading out and back in so the field keeps
     populating without anything ever crossing the pixel grid a fraction at a
     time. Redrawn a few times a second, which is all a breath needs. */
  function renderRest(step) {
    ...
    for (var i = 0; i < stars.length; i++) {
      ...
      if (st.dy) {
        st.fd -= step * 1.6;
        if (st.fd <= 0) {                     // gone: come back somewhere new
          st.fd = 0;
          st.dy = false;
          st.a = Math.random() * Math.PI * 2;
          st.r = 6 + Math.random() * (maxR - 8);
          st.c = Math.random() < 0.72 ? 0 : (Math.random() < 0.7 ? 1 : 2);
          st.tr = 0.5 + Math.random() * 1.4;
        }
      } else if (st.fd < 1) {
        st.fd = Math.min(1, st.fd + step * 1.2);
      }

      var mask = Math.min(1, Math.max(REST_FLOOR, (st.r / maxR - 0.06) / 0.32));
      var breath = 0.55 + 0.45 * (0.5 + 0.5 * Math.sin(st.tw));
      var a = REST_ALPHA * mask * breath * st.fd;
      var bx = cx + Math.cos(st.a) * st.r, by = cy + Math.sin(st.a) * st.r;
      block(bx, by, st.c, a);
      if (a > 0.62) {                       // the brightest few get a tick of glow
        block(bx + 1, by, st.c, a * 0.28);
        block(bx - 1, by, st.c, a * 0.28);
        block(bx, by + 1, st.c, a * 0.28);
        block(bx, by - 1, st.c, a * 0.28);
      }
    }

    /* retire roughly one star a second, so the field turns over slowly */
    if (stars.length && Math.random() < step) {
      var pick = stars[(Math.random() * stars.length) | 0];
      if (!pick.dy && pick.fd === 1) pick.dy = true;
    }
  ...
  function still() {
    ...
    canvas.classList.add('is-live');
    renderRest(0);                          // step 0: draws, advances nothing
  }
  ...
  window.addEventListener('resize', function () {
    if (!seeded) return;
    resize();
    if (!running) (resting ? renderRest(0) : render(elapsed, 0.016));
  });
  ...
  if (window.MutationObserver) {
    new MutationObserver(function () {
      readTheme();
      if (!running && seeded) (resting ? renderRest(0) : render(elapsed, 0.016));
