...
Cost and latency are design inputs, not bills you discover later. Three
structural decisions routinely cut spend 5–10x without touching quality:
stable prefix first, async where nobody is waiting, cheap model first.
...
The break-even is fast. With the 5-minute TTL, a prefix that is read just once
...
longer than five minutes, such as batch jobs, hourly crons, or a support inbox.
...
it. The API renders requests in a fixed order: `tools`, then `system`, then
`messages`. Together those two facts dictate prompt architecture:
...
- **Breakpoint placement:** you get at most 4 `cache_control` breakpoints. Put
  ...
  final (varying) block writes a distinct entry per request and reads none.
- **The 20-block lookback:** each breakpoint searches at most 20 content blocks
  ...
  miss, so add an intermediate breakpoint.

The prompt-caching doc holds two lesser-known techniques. **Pre-warm** by
...
never pays cold-prefix latency. For **parallel fan-out**, a cache entry
...
streamed token, then fire the other N−1 into the warm cache.
...
routinely exceeds five minutes. (`max_tokens: 0` pre-warming is rejected inside a batch.)
...
**Engineered** — run Haiku 4.5 first with a `confidence` field in the output
...
At 5x input price, output tokens are where prose costs real money. Three
concrete moves. Demand a JSON schema with terse enum values instead of
sentences (`"tier": "B"` beats "This lead appears to be of moderate quality
because..."). Set `max_tokens` to the actual ceiling of the schema for
...
logic but is not itself a doc claim.
