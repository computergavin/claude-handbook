...
- **Model and parameters** — exact model ID, temperature, max_tokens. "The prompt
  didn't change but behavior did" is usually a model or parameter change.
...
- **Correlation IDs** — a session/conversation ID and an anonymized user ID, so
...
that step 4 failed because step 2 retrieved garbage.

## The standard almost nobody knows exists

OpenTelemetry has semantic conventions for exactly this, covering spans,
metrics, and events for GenAI clients, agents, and MCP, now maintained in
their own repository, `open-telemetry/semantic-conventions-genai`. The
conventions' status is **Development**, so attribute names can still change,
...
and `gen_ai.tool.call.result` are all marked `Opt-In`, and the spec's default
posture is *don't record inputs and outputs at all*. A third pattern, storing
...
below on traces as a copy of user data is about to tell you.
...
The tool landscape fits in one paragraph. Langfuse is the self-host option.
Its core is MIT-licensed (`ee/`, `web/src/ee/`, and `worker/src/ee/` are not)
...
Span-level tracing, which gives you one trace per request linking the prompt
to the API calls and tool executions it triggered, is a separate beta. Set
both `CLAUDE_CODE_ENABLE_TELEMETRY=1` and `CLAUDE_CODE_ENHANCED_TELEMETRY_BETA=1`,
...
`BETA_TRACING_ENDPOINT`. Interactive CLI sessions additionally need the
...
transcript-logging `Stop` hook (see the Hooks chapter for that event): OTel
for the numbers, the hook for the replayable text.
...
happy path. The failures are what you were collecting traces for.
...
The minimum policy has three parts. Scrub credential-shaped strings (API keys,
`Bearer` tokens, JWTs, anything matching a `sk-`/`pk-` prefix) at write time,
...
Check dashboards weekly, but set alerts on the four signals that move first:
...
3. **Schema-validation failure rate.** If you validate outputs per the
