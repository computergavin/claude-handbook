...
leaving it to be inferred from an absent type: an `input_audio` block is accepted for
...
`xcrun simctl io booted screenshot shot.png` on the iOS simulator, or a
...
> send both images — labeled `Image 1:` baseline, `Image 2:` current — to a
...
> latency chapter for the resend arithmetic.

The limits worth knowing are 10 MB per image base64 on the Claude API (5 MB on
...
Two mechanics matter for coordinate work. Claude returns pixel coordinates
relative to the image *after* resizing, so pre-resize yourself if you need
coordinates to land on your original. And screenshots returned inside
`tool_result` blocks to the computer use and browser use toolsets are rejected
...
`"transformations": {"oversized_image": "error"}` on it. The setting lives
inside the `transformations` object, not as a bare key.
...
OCR-then-parse still wins in three cases. It wins on high-volume text-dense
...
approximate and small low-quality text invites hallucination.
...
Two tiers. Platform-native synthesis means `AVSpeechSynthesizer` on Apple
platforms (iOS 7+, macOS 10.14+). It is free, on-device, and offline, and
integration takes three lines: an `AVSpeechUtterance`, a voice, a rate. API
TTS, with OpenAI's `/v1/audio/speech` and `gpt-4o-mini-tts` as the reference
point, buys naturalness and steerability (an `instructions` parameter controls
accent, intonation, pacing) at the price of a network round-trip,
per-character billing, and an outage mode your offline app now owns.
...
turbo`. On the hosted side, OpenAI's `/v1/audio/transcriptions` takes files up
to 25 MB;
...
accuracy and streaming, and the `gpt-4o-transcribe-diarize` variant adds
speaker labels.
